...
In this block, I have written code to preform optimization using R's optim function.  Warning:  the call to optim requies a few minutes to finish. I have replaced parts of the code with a question mark.  Make the necessary changes to the code.  This block contains Q6 - Q8.
...
pheatmap(prob_net[c(1:20),c(1:20)],color=mypal,cluster_rows = F,cluster_cols = F,fontsize_number = 15,cex=0.75,cex.main=1,main="Probability")


library(smacof)


#### Q12:  Use the SMACOF package to preform MDS on dissimilarity matrix derived from prob_net (this is like Q5) setting ndim = 1,2,3,4,5,6.  Plot the stress of the fit associated with the different choices of ndim. (2 points)
a<-smacofSym(sim2diss(prob_net,method = 1),ndim =1)
b<-smacofSym(sim2diss(prob_net,method = 1),ndim =2)
c<-smacofSym(sim2diss(prob_net,method = 1),ndim =3)
d<-smacofSym(sim2diss(prob_net,method = 1),ndim =4)
e<-smacofSym(sim2diss(prob_net,method = 1),ndim =5)
f<-smacofSym(sim2diss(prob_net,method = 1),ndim =6)
plot(rbind(a$stress,b$stress,c$stress,d$stress,e$stress,f$stress),main="stress of the fit associated with the different choices of ndim")

barplot(cbind(a$stress,b$stress,c$stress,d$stress,e$stress,f$stress),main="stress of the fit associated with the different choices of ndim")
```
...
euclidistance<-function(x,x0){
return(sum((x-x0)^2))
}
out_dat

SSR<-function(data22){
centers<-matrix(NA,1,ncol(data22))
centers<-apply(data22,2,mean)
SSR=sum(apply(data22,1,euclidistance,centers))
return(SSR)
}

###group1:
MDS1<-res_mds$conf[c(1:25),]
Optimized1<-optimized_data[c(1:25),]
Network1<-b$conf[c(1:25),]
first<-rbind(SSR(MDS1),SSR(Optimized1),SSR(Network1))
first<-t(first)
###group2:
Second<-rbind(SSR(res_mds$conf[c(26:60),]),SSR(optimized_data[c(26:60),]),SSR(b$conf[c(26:60),]))
Second<-t(Second)
###group3:
Thrid<-rbind(SSR(res_mds$conf[c(61:78),]),SSR(optimized_data[c(61:78),]),SSR(b$conf[c(61:78),]))
Thrid<-t(Thrid)
###group4:
Fourth<-rbind(SSR(res_mds$conf[c(79:88),]),SSR(optimized_data[c(79:88),]),SSR(b$conf[c(79:88),]))
Fourth<-t(Fourth)
###group5:
Fifth<-rbind(SSR(res_mds$conf[c(89:93),]),SSR(optimized_data[c(89:93),]),SSR(b$conf[c(89:93),]))
Fifth<-t(Fifth)
##output:
table1<-as.data.frame(rbind(first,Second,Thrid,Fourth,Fifth))
names(table1)<-c("MDS data","optimized_data","netdata")
table1
...
####nn.dist an n x k matrix for the nearest neighbor Euclidean distances. nn.index an n x k matrix for the nearest neighbor indice.
library(FNN)

myLOF<-function(bbindex,bbdist)
{
#####numerator
matrixnumberator<-matrix(NA,1,5)
indexxixj<-bbindex[c(89:93),]
Dxixj<-bbdist[c(89:93),]
for(k in 1:5)
{
matrixnumberator[1,k]=sum(Dxixj[k,])
}
#####denominator
newdistance<-matrix(NA,1,5) ## 5 neighbor each have 5 distances
for(i in 1:5)
{
newdistance[,i]<-sum(bbdist[indexxixj[i,],])
denominator<-newdistance
}
myLOF<-1/5*(matrixnumberator/denominator)
return(myLOF)
}

bb<-get.knn(res_mds$conf,k=5)
aa<-myLOF(bb$nn.index,bb$nn.dist)
aa<-t(aa)
cc<-get.knn(optimized_data,k=5)
dd<-myLOF(cc$nn.index,cc$nn.dist)
dd<-t(dd)
ee<-get.knn(b$conf,k=5)
ff<-myLOF(ee$nn.index,ee$nn.dist)
ff<-t(ff)
gg<-get.knn(mypts,k=5)
hh<-myLOF(gg$nn.index,gg$nn.dist)
hh<-t(hh)
table2<-as.data.frame(cbind(aa,dd,ff,hh))
names(table2)<-c("MDS data","optimized_data","netdata","original data")
table2

#### Q15: Which dimension reduction method would you choose to study outliers in the lower dimensional embedding?  (1 point)

#### Base on the LOF result of Q14 and the mean of LOF
#### If the study purpose is to preserve the outlier to study in the lower dimensional embedding, I #### will choose the optimized dimensional embedding method,since it h
#### as the most similar LOF score with the original datat.Also it has the biggest SSR
##in Q14.( Means it preserve the outlier in the original dataset the best)
#### If the study purpose is to reduce outlier in the lower dimensional embedding,
##I will choose network dimension reduction method.(since it has the smallest LOF sc
##ore).Also it has the smallest SSR in Q14.
mean(table2[,1])
mean(table2[,2])
mean(table2[,3])
mean(table2[,4])
